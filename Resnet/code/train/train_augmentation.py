...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = to_pil_image(image)  # 🔹 Tensor → PIL 변환
...
test_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),  # ✅ `ToTensor()` 먼저 적용하여 PIL → Tensor 변환
    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
])
...
# ✅ TensorBoard 설정
writer = SummaryWriter(f"runs/ResNet50_Training_augmentation_{args.pretrained}_gpu_{args.gpu}_01_30")
...
if best_model_wts is not None:
    ...
    save_path = os.path.join(save_dir, f"best_resnet50_augmentation_{args.pretrained}_gpu_{args.gpu}.pth")
